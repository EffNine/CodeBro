...
// `crate::legacy::tools::run_tool_pipeline` + `call_ai_streaming`). It is marked
...
use crate::legacy::agent::memory_manager::MemoryConsolidationEngine;
use crate::legacy::agent::permissions::{PermissionDecision, PermissionLevel, PermissionManager};
use crate::legacy::agent::plan_memory::{PlanMemoryStore, PlanRecord};
use crate::legacy::agent::reflection::{ReflectionEngine, ReflectionStore};
use crate::legacy::agent::skill::SkillManager;
use crate::legacy::agent::skill::SkillStatus;
use crate::legacy::agent::workspace::{WorkspaceInfo, WorkspaceManager};
use crate::legacy::agent::Memory;
use crate::legacy::agent::Planner;
use crate::legacy::agent::SubAgent;
use crate::legacy::agent::TraceStore;
use crate::config::Config;
use crate::legacy::dispatcher::ToolDispatcher;
use crate::legacy::providers::Provider as _;
use crate::legacy::scanner::ProjectInfo;
use crate::legacy::tools::filesystem::{CreateFile, EditFile, ListFiles, ReadFile};
use crate::legacy::tools::git::{GitDiff, GitStatus};
...
async fn test_tool_dispatcher() {
    let registry = crate::legacy::dispatcher::ToolRegistry::new()
...
fn test_memory_task_lifecycle() {
    ...
    assert_eq!(
        ...
        crate::legacy::agent::memory::TaskStatus::InProgress
    );

    memory.complete_task(&task_id).unwrap();
    assert_eq!(
        memory.project.tasks[0].status,
        crate::legacy::agent::memory::TaskStatus::Completed
    );
}
...
fn test_memory_scoring() {
    let entry = crate::legacy::agent::memory::MemoryEntry {
...
fn test_skill_lifecycle_draft_to_testing() {
    let mut skill = crate::legacy::agent::skill::Skill::default();
...
fn test_trace_creation() {
    let trace = crate::legacy::agent::trace::create_trace(
        "task-1",
        "Add API endpoint",
...
fn test_trace_store_list_traces() {
    ...
    let trace1 = crate::legacy::agent::trace::create_trace(
        "task-4",
        "Request 1",
        "Plan 1",
        &["read_file".to_string()],
    ...
    let trace2 = crate::legacy::agent::trace::create_trace(
        "task-5",
        "Request 2",
        "Plan 2",
        &["run_command".to_string()],
...
fn test_subagent_creation() {
    let research = crate::legacy::agent::subagent::ResearchAgent::new();
...
fn test_subagent_planning_can_handle() {
    let planning = crate::legacy::agent::subagent::PlanningAgent::new();
...
fn test_subagent_coding_can_handle() {
    let coding = crate::legacy::agent::subagent::CodingAgent::new();
...
fn test_subagent_testing_can_handle() {
    let testing = crate::legacy::agent::subagent::TestingAgent::new();
...
fn test_subagent_review_can_handle() {
    let review = crate::legacy::agent::subagent::ReviewAgent::new();
...
fn test_task_router_simple_task() {
    let router = crate::legacy::agent::TaskRouter::new();
    let routing = router.route("Explain this function");
    assert!(matches!(
        routing,
        crate::legacy::agent::TaskRouting::DirectMainAgent
    ));
}
...
fn test_task_router_complex_task() {
    ...
    assert!(matches!(
        routing,
        crate::legacy::agent::TaskRouting::ParallelSubAgents(_)
    ));
}
...
fn test_task_router_moderate_task() {
    ...
    assert!(matches!(
        routing,
        crate::legacy::agent::TaskRouting::SequentialSubAgents(_)
    ));
}
...
fn test_task_router_analysis() {
    ...
    assert_eq!(analysis.complexity, crate::legacy::agent::TaskComplexity::Complex);
...
fn test_task_graph_creation() {
    let graph = crate::legacy::agent::TaskGraph::new("Add API feature");
...
fn test_task_graph_add_task() {
    let mut graph = crate::legacy::agent::TaskGraph::new("Add API feature");
...
fn test_task_graph_completion() {
    ...
    graph.update_status(&task1, crate::legacy::agent::TaskStatus::Completed);
    assert!(graph.is_complete());
}
...
fn test_task_graph_failure() {
    ...
    graph.update_status(&task1, crate::legacy::agent::TaskStatus::Failed);
...
fn test_smart_tool_router() {
    ...
    let router = crate::legacy::tools::SmartToolRouter::new(dispatcher);
...
fn test_experience_replay_creation() {
    let experience = crate::legacy::agent::ExperienceReplay::new();
    assert!(experience.is_ok());
}

#[test]
fn test_experience_record_and_retrieve() {
    let mut replay = crate::legacy::agent::ExperienceReplay::new().unwrap();

    let experience = crate::legacy::agent::Experience {
        id: uuid::Uuid::new_v4().to_string(),
        task_description: "Add authentication feature".to_string(),
        context: crate::legacy::agent::ExperienceContext {
        ...
        result: crate::legacy::agent::ExperienceResult {
            success: true,
...
fn test_agent_status_idle_default() {
    let state = crate::legacy::agent::status::AgentState::new("research");
    assert_eq!(state.status, crate::legacy::agent::AgentStatus::Idle);
...
fn test_agent_status_transitions() {
    let mut state = crate::legacy::agent::status::AgentState::new("coding");
    state.set_status(crate::legacy::agent::AgentStatus::Searching);
    assert!(state.status.is_active());
    assert!(state.started_at.is_some());

    state.set_status(crate::legacy::agent::AgentStatus::Completed);
...
fn test_agent_status_progress() {
    let mut state = crate::legacy::agent::status::AgentState::new("planning");
...
fn test_agent_status_reset() {
    let mut state = crate::legacy::agent::status::AgentState::new("testing");
    state.set_status(crate::legacy::agent::AgentStatus::Executing);
...
fn test_agent_status_monitor() {
    let mut monitor = crate::legacy::agent::status::AgentStatusMonitor::new();
    monitor.register_agent("research");
    monitor.register_agent("coding");
    assert_eq!(monitor.count(), 2);

    monitor.update_status("research", crate::legacy::agent::AgentStatus::Searching);
    ...
    assert_eq!(research.status, crate::legacy::agent::AgentStatus::Searching);
...
fn test_agent_event_bus() {
    let bus = crate::legacy::agent::events::AgentEventBus::new();
    let tx = bus.sender();

    tx.send(crate::legacy::agent::events::AgentEvent::AgentStarted {
    ...
    assert!(matches!(
        event,
        crate::legacy::agent::events::AgentEvent::AgentStarted { agent, .. } if agent == "research"
    ));
}

#[test]
fn test_agent_event_summary() {
    let event = crate::legacy::agent::events::AgentEvent::AgentCompleted {
        agent: "coding".to_string(),
        duration_ms: 5000,
    };
    assert_eq!(event.summary(), "Agent coding completed");

    let event = crate::legacy::agent::events::AgentEvent::AgentFailed {
        agent: "coding".to_string(),
...
fn test_agent_event_history() {
    let mut history = crate::legacy::agent::events::EventHistory::new(5);
    for i in 0..10 {
        history.record(crate::legacy::agent::events::AgentEvent::Log {
...
fn test_session_storage() {
    let dir = tempfile::tempdir().expect("tempdir");
    let store = crate::legacy::session::SessionStore::new(dir.path()).expect("store creation");
    let mut session = crate::legacy::session::Session::new("Add caching");
    session.record_event(&crate::legacy::agent::events::AgentEvent::AgentStarted {
        agent: "research".to_string(),
        task: "Find auth".to_string(),
    });
...
fn test_session_timeline_replay() {
    let mut session = crate::legacy::session::Session::new("Refactor auth");
    session.record_event(&crate::legacy::agent::events::AgentEvent::AgentStarted {
        agent: "research".to_string(),
        task: "Find".to_string(),
    });
    session.record_event(&crate::legacy::agent::events::AgentEvent::AgentCompleted {
        agent: "research".to_string(),
        duration_ms: 500,
    });
...
fn test_session_tracker_flow() {
    let dir = tempfile::tempdir().expect("tempdir");
    let mut tracker = crate::legacy::session::SessionTracker::new(dir.path()).expect("tracker creation");
    let id = tracker
    ...
    tracker
        .record_event(&crate::legacy::agent::events::AgentEvent::AgentStarted {
            agent: "coding".to_string(),
...
fn test_metrics_tracking() {
    let mut metrics = crate::legacy::metrics::TaskMetrics::new("Add caching");
...
fn test_cost_calculation_gpt4o() {
    let cost = crate::legacy::metrics::cost_for_tokens("gpt-4o", 1000000, 1000000);
    assert!((cost - 12.5).abs() < 0.01);
}

#[test]
fn test_cost_calculation_claude() {
    let cost = crate::legacy::metrics::cost_for_tokens("claude-sonnet-4", 1000000, 500000);
...
fn test_usage_history_persistence() {
    ...
    let mut tracker =
        crate::legacy::metrics::CostTracker::with_usage_path(usage_path.clone()).expect("cost tracker");
    tracker
    ...
    let reloaded = crate::legacy::metrics::CostTracker::with_usage_path(usage_path).expect("reload");
...
fn test_recovery_flow_test_failure() {
    let dir = tempfile::tempdir().expect("tempdir");
    let mut engine =
        crate::legacy::agent::recovery::RecoveryEngine::with_storage_path(dir.path().join("recovery.json"))
    ...
    assert_eq!(plan.action, crate::legacy::agent::RecoveryAction::AskCodingAgent);
...
fn test_recovery_flow_timeout() {
    ...
    assert_eq!(plan.action, crate::legacy::agent::RecoveryAction::Retry);
    assert!(plan.should_retry);
}
...
fn test_session_persistence_never_contains_secrets() {
    ...
    let store = crate::legacy::session::SessionStore::new(dir.path()).unwrap();
    let mut session = crate::legacy::session::Session::new("hardening task");
    let secret = "sk-session-secret-1234567890abcdef";
    session.record_event(&crate::legacy::agent::events::AgentEvent::ToolStarted {
        tool: "run_command".to_string(),
        args: format!("curl -H \"Authorization: Bearer {}\"", secret),
    });
    session.record_event(&crate::legacy::agent::events::AgentEvent::AgentFailed {
...
use crate::legacy::agent::subagent::{SubAgentCapability, SubAgentContext, SubAgentResult};
...
async fn test_coordinator_simple_task_emits_events() {
    let mut coordinator = crate::legacy::agent::AgentCoordinator::new(6);
    let events = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
    let ev = events.clone();
    let emit = move |e: crate::legacy::agent::AgentEvent| ev.lock().unwrap().push(e);
    ...
    let started = events
        .iter()
        .filter(|e| matches!(e, crate::legacy::agent::AgentEvent::AgentStarted { .. }))
        .count();
    let completed = events
        .iter()
        .filter(|e| matches!(e, crate::legacy::agent::AgentEvent::AgentCompleted { .. }))
        .count();
    let graph_updates = events
        .iter()
        .filter(|e| matches!(e, crate::legacy::agent::AgentEvent::TaskGraphUpdated { .. }))
        .count();
...
async fn test_coordinator_multi_agent_task() {
    ...
    let completed: Vec<&str> = events
        ...
        .filter_map(|e| match e {
            crate::legacy::agent::AgentEvent::AgentCompleted { agent, .. } => Some(agent.as_str()),
            _ => None,
        })
        .collect();
...
async fn test_coordinator_failure_routes_to_recovery() {
    ...
    let failed = events
        .iter()
        .filter(|e| matches!(e, crate::legacy::agent::AgentEvent::AgentFailed { .. }))
        .count();
    let recovery_logs = events
        .iter()
        .filter(
            |e| matches!(e, crate::legacy::agent::AgentEvent::Log { level, .. } if level == "coordination"),
        )
        .count();
...
async fn test_coordinator_report_is_grounded_in_repository() {
    ...
    assert!(
        report.contains("src/legacy/canonical_runtime/mod.rs"),
...
async fn test_coordinator_grounded_context_reaches_every_subagent() {
    use crate::legacy::agent::grounding::GroundingAssembler;
...
async fn real_provider_smoke_models_chat_tools() {
    ...
    let client = crate::legacy::providers::OpenAiProvider::new(config.clone());
    let reply = client
    ...
    let tools = vec![crate::legacy::providers::ToolDefinitionReexport {
...
struct RecordingOpenAiProvider {
    inner: crate::legacy::providers::OpenAiProvider,
...
impl RecordingOpenAiProvider {
    fn new(config: crate::config::Config) -> Self {
        RecordingOpenAiProvider {
            inner: crate::legacy::providers::OpenAiProvider::new(config),
...
impl crate::legacy::providers::Provider for RecordingOpenAiProvider {
    fn name(&self) -> &str {
        self.inner.name()
    }
    ...
    fn stream_response_with_tools(
        &self,
        message: &str,
        tools: &[crate::legacy::providers::ToolDefinitionReexport],
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<
                    Output = anyhow::Result<(String, Vec<crate::legacy::providers::StructuredToolCall>)>,
                > + Send
                + '_,
        >,
    > {
...
async fn real_provider_react_termination_repro() {
    ...
    for (label, task) in cases {
        for research_enabled in [false, true] {
            ...
            let mut runtime =
                crate::legacy::canonical_runtime::CanonicalRuntime::new_without_default_provider(
                    config, &root,
                )
                .expect("runtime");
            runtime.with_retry_policy(crate::legacy::provider_runtime::RetryPolicy::immediate(0));
            runtime.register_provider(provider);

            let events = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
            let sink = events.clone();
            let emit = move |e: crate::legacy::agent::events::AgentEvent| sink.lock().unwrap().push(e);
            let on_chunk = |_c: &str| {};
            let req = crate::legacy::canonical_runtime::TaskRequest {
                task,
                conversation: Vec::new(),
                emit: &emit,
                on_chunk: &on_chunk,
            };
            let mut options = crate::legacy::canonical_runtime::TaskOptions::default();
            ...
            let tool_starts: Vec<String> = events
                ...
                .iter()
                .filter_map(|e| match e {
                    crate::legacy::agent::events::AgentEvent::ToolStarted { tool, .. } => {
                        Some(tool.clone())
                    }
                    _ => None,
                })
...
fn picker_model(id: &str) -> crate::legacy::provider_manager::ModelInfo {
    crate::legacy::provider_manager::ModelInfo {
        id: id.to_string(),
        is_default: false,
        display_name: None,
        tool_calling: None,
...
mod validation {
    use super::*;
    use crate::legacy::agent::events::AgentEvent;
    ...
    use crate::legacy::dispatcher::ToolRegistry;
    use crate::legacy::providers::Provider;
    use crate::legacy::runtime::state::{RuntimeError, RuntimeState};
    use crate::tools::Tool;
    ...
    fn test_openai_provider_creation() {
        ...
        let provider = crate::legacy::providers::OpenAiProvider::new(config);
...
mod p2_reliability {
    ...
    fn test_provider_timeout_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("request timed out after 30s"),
    ...
    fn test_tool_timeout_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("command timed out after 60s"),
    ...
    fn test_rate_limit_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("429 Too Many Requests"),
    ...
    fn test_auth_failure_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("401 Unauthorized"),
    ...
    fn test_network_error_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("connection refused"),
    ...
    fn test_permission_denied_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("permission denied: Operation not permitted"),
    ...
    fn test_memory_limit_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("out of memory"),
    ...
    fn test_cancellation_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("operation cancelled"),
    ...
    fn test_tool_execution_error_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("command exited with code 1"),
    ...
    fn test_unknown_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error("some random error"),
    ...
    fn test_memory_sink() {
        ...
        assert_eq!(entries[0].level, crate::legacy::reliability::LogLevel::Info);
    ...
    fn test_log_levels() {
        ...
        let entries = sink.entries();
        assert_eq!(entries[0].level, crate::legacy::reliability::LogLevel::Trace);
        assert_eq!(entries[4].level, crate::legacy::reliability::LogLevel::Error);
    }
...
mod p25_validation {
    ...
    fn test_all_provider_timeout_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("provider request timeout"),
            RuntimeErrorCategory::ProviderTimeout
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("http request timed out"),
            RuntimeErrorCategory::ProviderTimeout
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("api deadline exceeded"),
            RuntimeErrorCategory::ProviderTimeout
        );
    }
    ...
    fn test_all_tool_timeout_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("operation timed out"),
            RuntimeErrorCategory::ToolTimeout
        );
    }
    ...
    fn test_all_rate_limit_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("rate limit exceeded"),
            RuntimeErrorCategory::ProviderRateLimit
        );
    }
    ...
    fn test_all_auth_failure_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("invalid API key"),
            RuntimeErrorCategory::ProviderAuthFailure
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("authentication failed"),
            RuntimeErrorCategory::ProviderAuthFailure
        );
    }
    ...
    fn test_all_network_error_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("DNS resolution failed"),
            RuntimeErrorCategory::ProviderNetworkError
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("network unreachable"),
            RuntimeErrorCategory::ProviderNetworkError
        );
    }
    ...
    fn test_all_permission_denied_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("EACCES: Permission denied"),
            RuntimeErrorCategory::ToolPermissionDenied
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("access denied"),
            RuntimeErrorCategory::ToolPermissionDenied
        );
    }
    ...
    fn test_all_memory_limit_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("oom-killer invoked"),
            RuntimeErrorCategory::SystemMemoryLimit
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("memory allocation failed"),
            RuntimeErrorCategory::SystemMemoryLimit
        );
    }
    ...
    fn test_all_cancellation_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("user cancelled"),
            RuntimeErrorCategory::SystemCancellation
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("shutdown requested"),
            RuntimeErrorCategory::SystemCancellation
        );
    }
    ...
    fn test_all_tool_execution_error_variations() {
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("tool execution failed"),
            RuntimeErrorCategory::ToolExecutionError
        );
    }
    ...
    fn test_unknown_classification() {
        assert_eq!(
            crate::legacy::reliability::classify_error(""),
            RuntimeErrorCategory::Unknown
        );
        assert_eq!(
            crate::legacy::reliability::classify_error("some random error"),
        ...
        assert_eq!(
            crate::legacy::reliability::classify_error("unexpected behavior"),
            RuntimeErrorCategory::Unknown
        );
    }
    ...
    fn test_runtime_error_display() {
        let err = crate::legacy::reliability::RuntimeError::new(
            "timeout",
            RuntimeErrorCategory::ProviderTimeout,
            "provider:test",
        );
    ...
    fn test_from_message() {
        let err = crate::legacy::reliability::from_message("connection refused", "main");
    ...
    fn test_error_is_standard_error() {
        let err =
            crate::legacy::reliability::RuntimeError::new("test", RuntimeErrorCategory::Unknown, "src");
...
mod p3_validation {
    use super::*;
    use crate::legacy::dispatcher::{ToolDispatcher, ToolRegistry};
    use crate::legacy::tools::{BuiltInProvider,CapabilityPermissionHook,DefaultRollbackHook,DiagnosticCollector,LifecycleManager,PermissionDecision,ProviderRegistry,RollbackHook,ToolDefinition,ToolHealth,ToolHooks,ToolLifecycleState,ToolMetadata,ToolProvider};
use crate::tools::{AsyncTool,StreamChunk,StreamResult,ToolCapabilities,ToolContext,ToolContextBuilder,ToolResult};
    ...
    fn p3_diagnostics_empty() {
        let diag = crate::legacy::tools::ToolDiagnostics::new("tool");
    ...
    fn p3_diagnostics_success() {
        let mut diag = crate::legacy::tools::ToolDiagnostics::new("tool");
    ...
    fn p3_regression_runtime_state_machine() {
        use crate::legacy::runtime::state::RuntimeState;
        let mut state = RuntimeState::Idle;
        state = state.try_transition(RuntimeState::Observing).unwrap();
        state = state.try_transition(RuntimeState::Reasoning).unwrap();
        state = state.try_transition(RuntimeState::Synthesizing).unwrap();
    ...
    fn p3_regression_existing_tools() {
        use crate::legacy::tools::{ListFiles};
use crate::tools::{RunCommand,Tool};
...
mod p55_validation {
    ...
    fn test_settings_navigation_sections() {
        ...
        let sm = crate::legacy::settings::SettingsManager::new(config, PathBuf::from("/tmp"));

        // Verify all 5 sections exist
        let sections = [
            crate::legacy::settings::SettingSection::General,
            crate::legacy::settings::SettingSection::Provider,
            crate::legacy::settings::SettingSection::Workspace,
            crate::legacy::settings::SettingSection::Features,
            crate::legacy::settings::SettingSection::Advanced,
        ];
    ...
    fn test_settings_pending_changes_workflow() {
        let config = Config::default_test();
        let mut sm = crate::legacy::settings::SettingsManager::new(config, PathBuf::from("/tmp"));
        ...
        assert_eq!(
            setting.value,
            crate::legacy::settings::SettingKind::String("gpt-4o-mini".to_string())
        );
    }
    ...
    fn test_settings_discard_workflow() {
        ...
        assert_eq!(
            model.value,
            crate::legacy::settings::SettingKind::String("".to_string())
        );
    }
    ...
    fn test_settings_persistence() {
        ...
        let mut sm = crate::legacy::settings::SettingsManager::new(config.clone(), config_dir.clone());
    ...
    fn test_provider_switching() {
        let mut pm = crate::legacy::provider_manager::ProviderManager::new(PathBuf::from("/tmp"));
        pm.register_builtin();
    ...
    fn test_provider_health_unknown_initially() {
        ...
        assert!(matches!(
            health,
            crate::legacy::provider_manager::HealthStatus::Unknown
        ));
    }
    ...
    fn test_provider_custom_registration() {
        ...
        pm.register_custom(
            crate::legacy::provider_manager::ProviderId::Custom("myprovider".to_string()),
    ...
    fn test_provider_connection_failure_handling() {
        ...
        assert!(matches!(
            result,
            Ok(crate::legacy::provider_manager::HealthStatus::Unhealthy { .. })
        ));
    }

    #[test]
    fn test_provider_wizard_flow_complete() {
        let mut wizard = crate::legacy::provider_manager::WizardState::new();

        // Step 1: Select provider
        wizard.select_provider(&crate::legacy::provider_manager::ProviderId::OpenAI);
        assert_eq!(
            wizard.step,
            crate::legacy::provider_manager::WizardStep::EnterApiKey
        );
        ...
        assert_eq!(
            wizard.step,
            crate::legacy::provider_manager::WizardStep::SelectModel
        );
    ...
    fn test_provider_persistence_roundtrip() {
        ...
        let mut pm = crate::legacy::provider_manager::ProviderManager::new(config_dir.clone());
        pm.register_builtin();
        pm.set_active("openai").unwrap();
        ...
        let mut loaded = crate::legacy::provider_manager::ProviderManager::new(config_dir.clone());
    ...
    fn test_git_detection() {
        ...
        let engine = crate::legacy::workspace_discovery::DiscoveryEngine::new(dir.path().to_path_buf());
        let discovery = engine.discover();

        assert!(discovery
            .findings
            .iter()
            .any(|f| matches!(f.kind, crate::legacy::workspace_discovery::DiscoveryKind::Git)));
    }
    ...
    fn test_docker_detection() {
        ...
        assert!(discovery
            .findings
            .iter()
            .any(|f| matches!(f.kind, crate::legacy::workspace_discovery::DiscoveryKind::Docker)));
    }
    ...
    fn test_makefile_detection() {
        ...
        assert!(discovery
            .findings
            .iter()
            .any(|f| matches!(f.kind, crate::legacy::workspace_discovery::DiscoveryKind::Make)));
    }
    ...
    fn test_cmake_detection() {
        ...
        assert!(discovery
            .findings
            .iter()
            .any(|f| matches!(f.kind, crate::legacy::workspace_discovery::DiscoveryKind::Cmake)));
    }
    ...
    fn test_discovery_kind_all_variants() {
        ...
        use crate::legacy::workspace_discovery::DiscoveryKind;
    ...
    fn test_runtime_detection_rust() {
        ...
        let scanner = crate::legacy::capability_discovery::CapabilityScanner::new(dir.path().to_path_buf());
        let discovery = scanner.scan();

        assert!(discovery.capabilities.iter().any(|c| {
            matches!(&c.kind, crate::legacy::capability_discovery::CapabilityKind::LanguageRuntime(r) if r == "rust")
                && c.available
        }));
    }
    ...
    fn test_runtime_detection_node() {
        ...
        assert!(discovery.capabilities.iter().any(|c| {
            matches!(&c.kind, crate::legacy::capability_discovery::CapabilityKind::LanguageRuntime(r) if r == "javascript")
                && c.available
        }));
    }
    ...
    fn test_build_tool_detection() {
        ...
        assert!(discovery.capabilities.iter().any(|c| {
            matches!(&c.kind, crate::legacy::capability_discovery::CapabilityKind::BuildSystem(b) if b == "cargo")
    ...
    fn test_testing_framework_detection() {
        ...
        assert!(discovery.capabilities.iter().any(|c| {
            matches!(&c.kind, crate::legacy::capability_discovery::CapabilityKind::TestingFramework(t) if t == "cargo_test")
    ...
    fn test_onboarding_first_run_detection() {
        let dir = tempdir().unwrap();
        let manager = crate::legacy::onboarding::OnboardingManager::new(dir.path().to_path_buf());
    ...
    fn test_onboarding_step_progression() {
        let mut manager = crate::legacy::onboarding::OnboardingManager::new(PathBuf::from("/tmp"));
        manager.start();

        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::Welcome
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::EnterApiKey
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::SelectProvider
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::DetectModel
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::DiscoverWorkspace
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::ReviewIntegrations
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::ReviewCapabilities
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::Confirm
        );

        manager.next();
        assert_eq!(
            manager.session.step,
            crate::legacy::onboarding::OnboardingStep::Complete
        );
    }
    ...
    fn test_onboarding_provider_selection() {
        ...
        manager.select_provider(&crate::legacy::provider_manager::ProviderId::OpenAI);
        assert_eq!(
            manager.session.wizard_state.selected_provider,
            Some(crate::legacy::provider_manager::ProviderId::OpenAI)
        );

        manager.select_provider(&crate::legacy::provider_manager::ProviderId::DeepSeek);
        assert_eq!(
            manager.session.wizard_state.selected_provider,
            Some(crate::legacy::provider_manager::ProviderId::DeepSeek)
        );
    }
    ...
    fn test_onboarding_workspace_discovery_integration() {
        ...
        manager.session.workspace_discovery = Some(
            crate::legacy::workspace_discovery::DiscoveryEngine::new(dir.path().to_path_buf()).discover(),
        );
    ...
    fn test_onboarding_capability_discovery_integration() {
        ...
        manager.session.capability_discovery = Some(
            crate::legacy::capability_discovery::CapabilityScanner::new(dir.path().to_path_buf()).scan(),
        );
    ...
    fn test_onboarding_completion_result() {
        ...
        let mut manager = crate::legacy::onboarding::OnboardingManager::new(dir.path().to_path_buf());
        manager.start();
    ...
    fn test_onboarding_step_info_all_steps() {
        let manager = crate::legacy::onboarding::OnboardingManager::new(PathBuf::from("/tmp"));

        // Test all step variants exist by testing through the flow
        let mut mgr = crate::legacy::onboarding::OnboardingManager::new(PathBuf::from("/tmp"));
        mgr.start();
    ...
    fn test_onboarding_is_complete_flag() {
        ...
        manager.session.step = crate::legacy::onboarding::OnboardingStep::Complete;
        assert!(manager.session.is_complete());
    }
    ...
    fn test_stress_repeated_capability_scans() {
        ...
        for _ in 0..50 {
            let scanner =
                crate::legacy::capability_discovery::CapabilityScanner::new(dir.path().to_path_buf());
            let discovery = scanner.scan();
    ...
    fn test_vision_platform_before_features() {
        ...
        let _registry = crate::legacy::dispatcher::ToolRegistry::new();
        let _provider = crate::legacy::providers::OpenAiProvider::new(Config::default_test());
        let _tool = crate::legacy::tools::ReadFile;
...
"#;
    ...
    fn test_edge_case_workspace_nested_dirs() {
        ...
        let engine = crate::legacy::workspace_discovery::DiscoveryEngine::new(nested.clone());
        let discovery = engine.discover();
    ...
    fn test_edge_case_settings_special_characters() {
        ...
        assert_eq!(
            setting.value,
            crate::legacy::settings::SettingKind::String(
    ...
    fn test_edge_case_discovery_no_permissions() {
        ...
        let result = std::panic::catch_unwind(|| {
            let engine = crate::legacy::workspace_discovery::DiscoveryEngine::new(PathBuf::from(
                "/nonexistent/path",
            ));
            engine.discover()
        });
...
mod p6_2_intent_engine {
    use super::*;
    use crate::legacy::intent_engine::ambiguity::AmbiguityDetector;
    use crate::legacy::intent_engine::classifier::IntentClassifier;
    use crate::legacy::intent_engine::confidence::ConfidenceModel;
    use crate::legacy::intent_engine::diagnostics::IntentDiagnostics;
    use crate::legacy::intent_engine::preview::ApprovalPreviewGenerator;
    use crate::legacy::intent_engine::resolver::IntentResolver;
    use crate::legacy::intent_engine::*;
    use std::collections::HashMap;
    use tokio::task;
...
mod p6_3_recommendation_engine {
    use super::*;
    use crate::legacy::intent_engine::{IntentCommand, IntentPlan, IntentType};
    use crate::legacy::recommendation_engine::diagnostics::RecommendationDiagnostics;
    use crate::legacy::recommendation_engine::engine::RecommendationEngine;
    use crate::legacy::recommendation_engine::filter::{
        filter, filter_by_confidence, filter_by_type, filter_by_uniqueness,
    };
    use crate::legacy::recommendation_engine::ranking::{deduplicate, full_rank, rank, remove_conflicts};
    use crate::legacy::recommendation_engine::rules::all_rules;
    use crate::legacy::recommendation_engine::*;
    ...
    fn test_generate_from_rules_dark_theme() {
        ...
        let recs = crate::legacy::recommendation_engine::rules::generate_from_rules(
            "Enable dark theme",
            "plan-1",
            &context,
        );
...
mod p6_4_workflow_engine {
    ...
    use crate::legacy::workflow_engine::dependency::{
        build_dependencies, calculate_depth, find_entry_points, find_exit_points, has_cycles,
    };
    use crate::legacy::workflow_engine::diagnostics::WorkflowDiagnostics;
    use crate::legacy::workflow_engine::ordering::{
        sort_by_priority, sort_by_stage_and_priority, topological_sort,
    };
    use crate::legacy::workflow_engine::planner::WorkflowPlanner;
    use crate::legacy::workflow_engine::preview::{
        generate_approval_summary, generate_compact_preview, generate_preview,
    };
    use crate::legacy::workflow_engine::validator::{generate_warnings, validate_inputs, validate_plan};
    use crate::legacy::workflow_engine::*;
...
mod p6_5_adaptive_validation {
    use super::*;
    use crate::legacy::adaptive_validation::confidence::ConfidenceEvaluator;
    use crate::legacy::adaptive_validation::diagnostics::AdaptiveDiagnostics;
    use crate::legacy::adaptive_validation::engine::AdaptiveValidationEngine;
    use crate::legacy::adaptive_validation::policy::{default_policies, PolicyEngine};
    use crate::legacy::adaptive_validation::risk::RiskAssessor;
    use crate::legacy::adaptive_validation::rules::{all_rules, evaluate_all};
    use crate::legacy::adaptive_validation::validator::Validator;
    use crate::legacy::adaptive_validation::*;
    use crate::legacy::intent_engine::{IntentCommand, IntentPlan, IntentType};
    use crate::legacy::workflow_engine::{ExecutionStrategy, WorkflowPlan};
...
mod p7_concurrency_validation {
    ...
    use crate::legacy::integration_pipeline::IntegrationPipeline;
    use crate::legacy::intent_engine::{IntentClassifier, IntentPlan, IntentResolver, IntentType};
    use crate::legacy::preference_engine::{
    ...
    use crate::legacy::workflow_engine::{WorkflowDiagnostics, WorkflowPlanner, WorkflowResult};
    ...
    fn test_workflow_planner_thread_safe() {
        ...
        for i in 0..10 {
            ...
            let plan = IntentPlan::new(
                ...
                "Model change",
                vec!["Rule match".to_string()],
                vec![crate::legacy::intent_engine::IntentCommand::UpdateModelPreference {
                    key: "model".to_string(),
    ...
    fn test_integration_pipeline_thread_safe() {
        ...
        prefs.add(crate::legacy::preference_engine::Preference::new(
            "model",
