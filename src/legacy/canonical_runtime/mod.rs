...
use crate::legacy::agent::coordinator::AgentCoordinator;
use crate::legacy::agent::events::AgentEvent;
use crate::legacy::agent::grounding::GroundedContext;
use crate::legacy::agent::recovery::RecoveryEngine;
use crate::legacy::agent::status::AgentStatus;
use crate::legacy::agent::task_graph::{TaskGraph, TaskStatus};
use crate::legacy::agent::tool_parser::{self, ToolCall};
use crate::legacy::assembly::{
...
use crate::legacy::engineering_context::constraints::{ConstraintCategory, EngineeringConstraint};
use crate::legacy::engineering_context::workspace::WorkspaceFile;
use crate::legacy::engineering_context::{
...
use crate::legacy::engineering_objective::{
...
use crate::legacy::prompt_builder::{CompiledPrompt, PromptBuilder};
use crate::legacy::provider_runtime::routing::ProviderRoutingDecision;
use crate::legacy::provider_runtime::{
...
use crate::legacy::providers::{OpenAiProvider,StructuredToolCall,ToolDefinition};
use crate::legacy::scanner::ProjectInfo;
use crate::tools::shell::redact_secrets_public;
use crate::legacy::tools::{detect_workspace_root,is_toolable,run_tool_pipeline};
use crate::legacy::workspace_runtime::{LocalFileSystem, WorkspaceRuntime};
...
fn task_deadline_budget(opts: &TaskOptions) -> u64 {
    ...
    if opts.research_enabled {
        budget = budget.saturating_add(crate::legacy::research::ResearchLimits::default().timeout_ms);
    }
    if opts.testing_enabled {
        budget = budget.saturating_add(crate::legacy::testing::TestingLimits::default().timeout_ms);
    }
    if opts.planning_enabled {
        budget = budget.saturating_add(crate::legacy::planning::PlanningLimits::default().timeout_ms);
    }
    if opts.coding_enabled {
        budget = budget.saturating_add(crate::legacy::coding_limits::CodingLimits::default().timeout_ms);
    }
    if opts.review_enabled {
        budget = budget.saturating_add(crate::legacy::review::ReviewLimits::default().timeout_ms);
    }
    budget
}
...
pub struct TaskOptions {
    ...
    /// [`crate::legacy::coding_contract::CodingResult`] into the main LLM context. Defaults to
    /// false.
    ...
    /// [`crate::legacy::review::ReviewResult`] into the main LLM context. Failure is
...
pub struct CanonicalRuntime {
    ...
    io_providers: HashMap<ProviderId, Arc<dyn crate::legacy::providers::Provider>>,
...
impl CanonicalRuntime {
    ...
    pub fn register_provider(&mut self, provider: Arc<dyn crate::legacy::providers::Provider>) {
    ...
    pub async fn run_task_with_options(
        ...
        let memory_entries: Vec<String> = memory_ctx
            ...
            .collect();
        let grounded = crate::legacy::agent::grounding::GroundingAssembler::new(&self.workspace_root)
        ...
        let mut research_result: Option<crate::legacy::research::ResearchResult> = None;
        if opts.research_enabled {
            let research = self
            ...
            match research {
                Ok(result) => {
                    ...
                    diag.research = Some(
                        crate::legacy::canonical_runtime::diagnostics::ResearchDiagnostics::from(
                            result.clone(),
                        ),
                    );
                    ...
                    context = extend_context(
                        ...
                        vec![ContextFragment {
                            ...
                            structured_facts: Some(
                                crate::legacy::engineering_context::context::StructuredFacts::new(
                                    "research",
                                )
        ...
        let mut testing_result: Option<crate::legacy::testing::TestingResult> = None;
        if opts.testing_enabled {
            let testing = self
            ...
            match testing {
                Ok(result) => {
                    let render = result.render();
                    diag.testing = Some(
                        crate::legacy::canonical_runtime::diagnostics::TestingDiagnostics::from(
                            result.clone(),
                        ),
                    );
        ...
        let mut planning_result: Option<crate::legacy::planning::PlanningResult> = None;
        if opts.planning_enabled {
            let planning = self
            ...
            match planning {
                Ok(result) => {
                    let render = result.render();
                    diag.planning = Some(
                        crate::legacy::canonical_runtime::diagnostics::PlanningDiagnostics::from(
                            result.clone(),
                        ),
                    );
        ...
        let mut coding_result: Option<crate::legacy::coding_contract::CodingResult> = None;
            ...
            if planning_result.is_none() {
                ...
                match coding {
                    Ok(result) => {
                        let render = result.render();
                        diag.coding = Some(
                            crate::legacy::canonical_runtime::diagnostics::CodingDiagnostics::from(
                                result.clone(),
                            ),
                        );
        ...
        if opts.review_enabled {
            ...
            match review {
                Ok(result) => {
                    let render = result.render();
                    diag.review = Some(
                        crate::legacy::canonical_runtime::diagnostics::ReviewDiagnostics::from(
                            result.clone(),
                        ),
                    );
                    context = extend_context(
                        ...
                        vec![ContextFragment {
                            ...
                            relevance_score: 0.85,
                            structured_facts: Some(
                                crate::legacy::engineering_context::context::StructuredFacts::new("review")
                                    .with_field("findings_count", result.findings.len())
                    ...
                    let critical = result
                        .findings
                        .iter()
                        .filter(|f| f.severity == crate::legacy::review::ReviewSeverity::Critical)
                        .count();
                    if result.verdict == crate::legacy::review::ReviewVerdict::Fail || critical > 0 {
                        return self.fail(
                            req,
                            Some(&mut graph),
                            &root_id,
                    ...
                    if result.verdict == crate::legacy::review::ReviewVerdict::PassWithRisks {
            ...
            Ok(result) => {
                ...
                context = extend_context(
                    ...
                    vec![ContextFragment {
                        ...
                        relevance_score: 0.85,
                        structured_facts: Some(
                            crate::legacy::engineering_context::context::StructuredFacts::new("research")
                                .with_field("files_inspected", result.files_inspected.len())
                                .with_field("symbols_found", result.symbols_found.len())
                                .with_field("findings_count", result.findings.len())
                                .with_field("termination", result.termination.to_string())
            ...
            Ok(result) => {
                ...
                context = extend_context(
                    ...
                    vec![ContextFragment {
                        ...
                        relevance_score: 0.85,
                        structured_facts: Some(
                            crate::legacy::engineering_context::context::StructuredFacts::new("testing")
                                .with_field("commands_run_count", result.commands_run.len())
                                .with_field("failures_count", result.failures.len())
                                .with_field(
                                    "exit_codes",
            ...
            Ok(result) => {
                ...
                context = extend_context(
                    ...
                    vec![ContextFragment {
                        ...
                        relevance_score: 0.85,
                        structured_facts: Some(
                            crate::legacy::engineering_context::context::StructuredFacts::new("planning")
                                .with_field("steps_count", result.plan.len())
                                .with_field("affected_files_count", result.affected_files.len())
                                .with_field("affected_symbols_count", result.affected_symbols.len())
                                .with_field("risks_count", result.risks.len())
            ...
            Ok(result) => {
                ...
                context = extend_context(
                    ...
                    vec![ContextFragment {
                        ...
                        relevance_score: 0.85,
                        structured_facts: Some(
                            crate::legacy::engineering_context::context::StructuredFacts::new("coding")
                                .with_field("changes_count", result.changes.len())
                                .with_field(
                                    "verified_changes_count",
                                    result.changes.iter().filter(|c| c.verified).count(),
    ...
    ) -> std::result::Result<crate::legacy::research::ResearchResult, String> {
        let tooling = build_research_tooling(&self.workspace_root);
        let mut subagent = crate::legacy::research::ResearchSubagent::new(
            self.provider_runtime.clone(),
            self.router.clone(),
            self.io_providers.clone(),
            tooling,
        );
        let request = crate::legacy::research::ResearchRequest::new(task, self.workspace_root.clone())
    ...
    async fn run_autonomous_research(
        &self,
        req: &TaskRequest<'_>,
        grounded: std::sync::Arc<crate::legacy::agent::grounding::GroundedContext>,
        opts: &TaskOptions,
    ) -> std::result::Result<crate::legacy::research::ResearchResult, String> {
    ...
    pub async fn run_testing_task(
        ...
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::testing::TestingResult, String> {
    ...
    pub async fn run_testing_task_with_limits(
        ...
        task: &str,
        grounding: GroundedContext,
        limits: crate::legacy::testing::TestingLimits,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::testing::TestingResult, String> {
        let tooling =
            crate::legacy::testing::TestingTooling::new(&self.workspace_root, limits.command_timeout_secs);
        let mut subagent = crate::legacy::testing::TestingSubagent::new(
            self.provider_runtime.clone(),
            self.router.clone(),
            self.io_providers.clone(),
            tooling,
        );
        let request = crate::legacy::testing::TestingRequest::new(task, self.workspace_root.clone())
            .with_grounding(grounding)
            .with_limits(limits);
        Ok(subagent.run(request, emit, cancel).await)
    }
    ...
    pub async fn run_planning_task(
        ...
        task: &str,
        grounding: GroundedContext,
        research: Option<crate::legacy::research::ResearchResult>,
        testing: Option<crate::legacy::testing::TestingResult>,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::planning::PlanningResult, String> {
        let tooling = build_planning_tooling(&self.workspace_root);
        let mut subagent = crate::legacy::planning::PlanningSubagent::new(
            self.provider_runtime.clone(),
            self.router.clone(),
            self.io_providers.clone(),
            tooling,
        );
        let request = crate::legacy::planning::PlanningRequest::new(task, self.workspace_root.clone())
            .with_grounding(grounding)
    ...
    pub async fn run_planning_task_with_limits(
        ...
        research: Option<crate::legacy::research::ResearchResult>,
        testing: Option<crate::legacy::testing::TestingResult>,
        limits: crate::legacy::planning::PlanningLimits,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::planning::PlanningResult, String> {
        let tooling = build_planning_tooling(&self.workspace_root);
    ...
    pub async fn run_coding_task(
        ...
        task: &str,
        grounding: GroundedContext,
        planning: Option<crate::legacy::planning::PlanningResult>,
        research: Option<crate::legacy::research::ResearchResult>,
        testing: Option<crate::legacy::testing::TestingResult>,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::coding_contract::CodingResult, String> {
        self.run_coding_task_with_limits(
            ...
            research,
            testing,
            crate::legacy::coding_limits::CodingLimits::default(),
            emit,
            cancel,
        )
        .await
    ...
    pub async fn run_coding_task_with_limits(
        ...
        research: Option<crate::legacy::research::ResearchResult>,
        testing: Option<crate::legacy::testing::TestingResult>,
        limits: crate::legacy::coding_limits::CodingLimits,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::coding_contract::CodingResult, String> {
        ...
        let tooling = crate::legacy::coding_tooling::CodingTooling::new(
            &self.workspace_root,
            &planned_files,
            strict,
            limits.command_timeout_secs,
        );
        let mut subagent = crate::legacy::coding_runtime::CodingSubagent::new(
            self.provider_runtime.clone(),
            self.router.clone(),
            self.io_providers.clone(),
            tooling,
        );
        let request = crate::legacy::coding_contract::CodingRequest::new(task, self.workspace_root.clone())
            .with_grounding(grounding)
            .with_research(research)
            .with_testing(testing)
    ...
    /// [`crate::legacy::review::ReviewResult`].
    pub async fn run_review_task(
        ...
        testing: Option<crate::legacy::testing::TestingResult>,
        planning: Option<crate::legacy::planning::PlanningResult>,
        coding: Option<crate::legacy::coding_contract::CodingResult>,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::review::ReviewResult, String> {
        self.run_review_task_with_limits(
            ...
            planning,
            coding,
            crate::legacy::review::ReviewLimits::default(),
            emit,
            cancel,
        )
        .await
    ...
    pub async fn run_review_task_with_limits(
        ...
        planning: Option<crate::legacy::planning::PlanningResult>,
        coding: Option<crate::legacy::coding_contract::CodingResult>,
        limits: crate::legacy::review::ReviewLimits,
        emit: &(dyn Fn(AgentEvent) + Send + Sync),
        cancel: Option<crate::cancellation::CancellationToken>,
    ) -> std::result::Result<crate::legacy::review::ReviewResult, String> {
        let tooling = crate::legacy::review::ReviewTooling::new(&self.workspace_root);
        let mut subagent = crate::legacy::review::ReviewSubagent::new(
            self.provider_runtime.clone(),
            self.router.clone(),
            self.io_providers.clone(),
            tooling,
        );
        let request = crate::legacy::review::ReviewRequest::new(task, self.workspace_root.clone())
            .with_grounding(grounding)
            .with_research(research)
            .with_testing(testing)
            .with_planning(planning)
...
fn build_tool_registry(workspace_root: &Path) -> ToolRegistry {
    ...
    ToolRegistry::new()
        .register(Arc::new(crate::legacy::tools::ListFiles))
        .register(Arc::new(crate::legacy::tools::ReadFile))
        .register(Arc::new(crate::legacy::tools::CreateFile))
        .register(Arc::new(crate::legacy::tools::EditFile))
        .register(Arc::new(
            crate::tools::RunCommand::new()
                .with_working_directory(root.to_string_lossy().to_string()),
        ))
        .register(Arc::new(crate::legacy::tools::GitStatus))
        .register(Arc::new(crate::legacy::tools::GitDiff))
        .register(Arc::new(crate::legacy::tools::PlaywrightTool::new(root)))
}
...
fn build_research_tooling(workspace_root: &Path) -> crate::legacy::research::ResearchTooling {
    crate::legacy::research::ResearchTooling::new(workspace_root)
}
...
fn build_planning_tooling(workspace_root: &Path) -> crate::legacy::planning::PlanningTooling {
    crate::legacy::planning::PlanningTooling::new(workspace_root)
}
...
fn dedup_fragments(fragments: &mut Vec<ContextFragment>) {
    ...
    fragments.retain(|f| {
        ...
        seen.insert(crate::legacy::assembly::sources::fragment_fingerprint(
            &f.source, &f.content,
        ))
    });
}
