...
use crate::legacy::canonical_runtime::execution;
use crate::legacy::canonical_runtime::TaskOptions;
use crate::legacy::provider_runtime::routing::IntelligentProviderRouter;
use crate::legacy::provider_runtime::{Capability, ProviderId, ProviderRuntime, RouteRequest};
use crate::legacy::providers::StructuredToolCall;
use crate::legacy::research::contract::truncate_chars;

use super::coding_contract::{
    AppliedChange, CodingObservation, CodingRequest, CodingResult, CodingTermination,
    VerificationRecord, VerificationSource,
};
use super::coding_limits::CodingLimits;
use crate::legacy::coding_tooling::{self, parse_proposal_args, CodingTooling};
...
impl CodingSubagent {
    ...
    pub async fn run(
        ...
        loop {
            ...
            let tool_defs: Vec<crate::legacy::providers::ToolDefinitionReexport> = {
    ...
    fn propose(&self, state: &mut CodingState, arguments: &str) -> (String, bool) {
        ...
        (
            crate::legacy::coding_tooling::truncate_and_redact(&result, state.limits.max_tool_result_chars),
            true,
        )
    }
    ...
    async fn verify(
        ...
        let truncated =
            crate::legacy::coding_tooling::truncate_and_redact(&record.render(), state.limits.max_tool_result_chars);
        (record, truncated)
    }
    ...
    async fn completion_gate(
        ...
        for command in &commands {
            ...
            state.observe(CodingObservation {
                ...
                result: crate::legacy::coding_tooling::truncate_and_redact(
                    &record.render(),
...
struct CodingState {
    ...
    git_before: Option<crate::legacy::testing::GitStateSnapshot>,
    git_after: Option<crate::legacy::testing::GitStateSnapshot>,
}
...
mod tests {
    ...
    fn test_render_testing_preserves_cancelled_machine_fact() {
        ...
        let cancelled = crate::legacy::testing::TestCommandResult {
        ...
        state.request.testing = Some(crate::legacy::testing::TestingResult {
            summary: String::new(),
            findings: Vec::new(),
            ...
            iterations: 1,
            model_calls: 1,
            termination: crate::legacy::testing::TestingTermination::Cancelled,
            synthesis_complete: false,
            observations: Vec::new(),
            limitations: Vec::new(),
            duration_ms: 0,
