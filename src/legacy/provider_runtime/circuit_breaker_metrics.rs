...
use crate::legacy::observability::{
...
impl CircuitBreakerMetricsCollector {
        ...
        match (from, to) {
            (CircuitBreakerState::Closed, CircuitBreakerState::Open) => {
                if let Some(ref eb) = self.event_bus {
                    ...
                    bus.emit(&crate::legacy::observability::event::error_event(
                        CorrelationId::new(),
            ...
            (CircuitBreakerState::Open, CircuitBreakerState::HalfOpen) => {
                if let Some(ref eb) = self.event_bus {
                    let bus = eb.lock().unwrap();
                    bus.emit(&crate::legacy::observability::event::pipeline_completed(
                        CorrelationId::new(),
                        0,
                        "breaker_half_open",
                        1,
    ...
    fn emit_event(&self, provider: &ProviderId, correlation_id: &str, outcome: &str) {
        if let Some(ref eb) = self.event_bus {
            ...
            let event = crate::legacy::observability::event::error_event(
                CorrelationId::new(),
                "CircuitBreaker",
                &format!(
...
mod tests {
    use super::*;
    use crate::legacy::observability::{EventBus, MetricRecorder};
    use std::sync::Arc;
