...
### Added
- **Portable memory export/import (WS5)** — explicit, verified, redacted portability for durable user context, history, and skills. No new MCP tool (surface stays 24), no schema change. `codebro export --out <dir>` writes a versioned `manifest.json` (format version, per-table counts, column lists, SHA-256 `data_hash`) plus one JSONL file per table — the same layout as the pre-existing `~/memory/export` mirror — with every string value passed through the canonical secret-redaction authority (skill-version content hashes are recomputed over redacted content so bundles stay self-consistent). `codebro import --file <dir> [--root <ws>] [--map OLD=NEW] [--dry-run] [--no-publish] [--origin <label>]` verifies integrity first (strict hash for v1 bundles; legacy manifests without a format version fall back to count verification and report `counts_verified`), validates every row before the first write (any malformed/corrupted row aborts with nothing written), remaps workspace roots explicitly, remaps evidence citations to local events (an unresolvable citation refuses the record — provenance is never fabricated), preserves authority verbatim (a `user_confirmed` record stays confirmed; an `ai_inferred` record never becomes confirmed; a confirmed local record is never downgraded), never overwrites newer local state (equal-timestamp differences are reported conflicts; older imports are skipped), and inserts skills and candidates verbatim without any lifecycle transition (safe names, known scope/status vocabulary, content hashes revalidated; missing SKILL.md artifacts for active skills are published atomically and never over a differing file). Import is idempotent — records upsert by id, events dedup by a content-addressed key, skills insert-if-absent — so a re-run converges. History (sessions/events) is imported verbatim even when its workspace is not mapped, so citations from imported global records always resolve; project-scoped records and skills for unmapped workspaces are skipped and reported. Tests: 19 portability unit tests + `crates/mcp-server/tests/portability_e2e.rs` (4 real-CLI probes); the live `~/memory` mirror imports cleanly (2 global records, 40 events, 11 sessions; project rows skipped without a mapping) and a second import is a no-op. See `docs/design/PORTABILITY.md`.
- **Intent guard (WS4)** — deterministic wrong-project / wrong-context safety layer in context assembly. No new MCP tool, no schema change. After fingerprint resolution, a pure guard (`crates/mcp-server/src/intent_guard.rs`) evaluates the packet viewpoint against the workspace basename, the loaded project identity, the store's other known workspaces, the task text, and record scope: global records that name exactly one other known workspace and not the current project are excluded; records naming the current project plus another (or several others) are flagged, never dropped; intents are never excluded (only flagged); project/task-scoped records are never content-scanned (scope is authoritative). The bounded, explainable `guard` report (verdict `aligned|review|unverified`, `warn`/`info` signals, excluded/flagged record ids, intent coverage) travels with the `context` packet and the `engineering_brief`; a `review` verdict also adds a bounded `notes` warning. Matching is normalized-identifier lexical, deterministic (no clocks, randomness, embeddings, or LLM), read-only (never writes, never promotes authority), and never echoes record content (ids and workspace basenames only). The OpenCode session plugin renders a bounded `Intent guard: review — …` line and per-record `guard:` annotations. Tests: 16 guard unit tests + 8 packet integration tests + 1 store query test + 1 brief propagation test + 1 plugin probe. See `docs/design/INTENT_GUARD.md`.
- **Session lifecycle via `recall` (WS3)** — sessions gain explicit, deterministic lifecycle without a new tool, daemon, or automation. `recall` gains `action`: `sessions` (read-only inventory: title/status/staleness/has_summary, optional status/task filter, bounded), `title` (name a session; bounded, secret-redacted, idempotent — `updated_at` is never bumped so staleness keeps its meaning), and `close` (finalize with terminal `completed`/`abandoned`, optional bounded `reason`, and an optional bounded `summary` persisted as a schema-tagged `context_snapshot_json` snapshot for future recall). Status and snapshot land in one atomic UPDATE; a repeated close returns the durable ending unchanged (no rewrite, no duplicate history); malformed/legacy snapshots degrade to "no summary" instead of failing recall; closed sessions remain searchable and their summaries surface as bounded `session_summary` excerpts. One structural `SessionEnded` history event is appended per close (dedup-keyed by session id; a failed append never invalidates the close). Lifecycle writes never create context records or USER_CONFIRMED facts, and serialize on the workspace mutation lock; `search` (the unchanged default) and `sessions` stay read-only. No schema change: `context_snapshot_json` has existed since v1; `SessionRecord` carries it internally without serializing it. Tests: 6 context-runtime tests (title stability/bounds/redaction, snapshot bounds/optionality, malformed snapshots, record separation, legacy NULL/corrupt rows, closed-session recall summary) + 5 MCP integration tests (`session_lifecycle_tests`: title idempotency/refusals/redaction, atomic close + idempotent second close + no duplicate events + closed-session recall + no record creation, close input refusals, backward-compatible read-only search, session filtering/bounds).
- **OpenCode integration package (`integrations/opencode/`)** — host-driven session-start context injection. An OpenCode plugin (`plugins/codebro-context.js`) calls the read-only `codebro context` CLI and injects a bounded, provenance-labelled digest (workspace orientation, execution state, confirmed intents/decisions/memory) into the system prompt once per session, rebuilding when the first user message arrives so ranking uses the real task; the digest is also injected before compaction. Fail-open by design: missing binary, non-zero exit, bad JSON, or timeout means no injection, never a blocked session. Workspace-gated on `.codebro/facts.json` (`CODEBRO_CONTEXT_ALWAYS=1` overrides); env-configurable (`CODEBRO_BIN`, `CODEBRO_CONTEXT_DISABLED`, `CODEBRO_CONTEXT_TIMEOUT_MS`, `CODEBRO_CONTEXT_MAX_CHARS`). No daemon, no new MCP tool, no schema or state change; CodeBro remains pull-only. Tests: `integrations/opencode/test/codebro-context.test.mjs` (11 probes, stubbed binary).
- **Host context CLI verb (`codebro context`)** — read-only host/hook surface: prints the bounded engineering context packet through the same shared assembly (`crates/mcp-server/src/context_packet.rs`) as the MCP `context` tool, so external hosts (an OpenCode plugin, cron jobs, scripts) can inject session-start context without an MCP client. Flags: `--root`, `--task` (omit for the structural digest), repeatable `--keyword`, optional `--task-id`, `--pretty`. Identical retrieval semantics, budgets, and provenance labels; stdout carries exactly one JSON document. No tool-surface, schema, or state change; the MCP handler and the CLI now delegate to one implementation. Tests: `crates/mcp-server/tests/cli_context_e2e.rs` (4 probes).
- **Jev escalation advisory (informational-only, off by default)** — the locked `jev-baseline-1` (`jev-1.13.0`, question set v3, confidence ≥ 0.80, escalation-only) now operates as a real CodeBro advisory component through one live seam: the skill `request_approval` handler observes post-decision via a detached task whose outcome is discarded. No new MCP tool, no schema change, no production authority: Jev cannot approve, deny, execute, retry, or change policy/task/sandbox state. Baseline drift (model, questions, schema, threshold, scope, authority) disables advisory automatically via `verify_baseline()` on every evaluation. Flags `JEV_SHADOW_ENABLED` / `JEV_ADVISORY_ENABLED` both default `false` (both off = zero Jev calls, unchanged behavior); rollback is `JEV_ADVISORY_ENABLED=false`. Output is a `JEV ADVISORY: … Informational only.` tracing line plus structured telemetry (`baseline_id`, hashes, confidence, latency, tokens, provenance) in `.codebro/jev-advisory.jsonl`; secrets never logged. See `docs/JEV_ADVISORY.md`. Tests: new `codebro_integration` (11) and `jev_codebro_integration` (3) suites; full workspace green with no regressions.
- **Execution-state reliability gate (edit → verification → completion)** — closes the failure class where an agent claims an implementation succeeded although the edit failed, applied partially, produced invalid code, or left the build/tests broken. No new MCP tool, no schema change (stays v7).
...
### Removed
- **Standalone `impact_analyze` MCP tool** — measured non-use: 0 agent calls in 19 sessions across ab-v2/Phase-1/1b/2, including two tasks designed for it (ab-v2 T3, Phase-2 T8); agents consistently solve impact questions via reading, grep, memory/recall, or the brief. Surface goes 25 → 24 tools; contract, acquisition flow, and inventories updated. RETAINED: the `impact-engine` crate and `crate::impact::analyze` engine (brief's embedded impact section with risk signals, doctor/debugging paths, `HistoryKind::ImpactAnalyzed` records). No schema change. See `eval/IMPACT_ANALYZE_REMOVAL_PROPOSAL.md` (approved by operator). Regressions updated, none weakened: tool-count assertions now pin 24; P6 risk coverage re-pointed at the brief's embedded impact section; final-gate absurd-depth probe re-pointed at brief validation; 15 tool-level impact tests removed (engine behavior remains covered by 46 impact-engine unit tests).
