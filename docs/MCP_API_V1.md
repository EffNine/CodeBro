...
| `recall` | v1.3-dev (P2, `docs/evolution/P2_IMPLEMENTATION.md`) | `query` (required for search, ≥1 searchable token), `action?` (search default \| sessions \| title \| close), `scope?` (project default \| task + `task_id?` \| global opt-in), `kinds?` (taxonomy subset), `session_id?`, `title?`/`status?`/`reason?`/`summary?` (lifecycle writes and session filtering), `limit?` (default 10, max 50; sessions: default 20), `workspace_root?` | historical evidence on demand plus explicit session lifecycle. `search` (default) and `sessions` are read-only and write nothing: session-grouped bounded excerpts (decision/failure/validation/change) with session, timestamp, scope, task, event type, source, stale flag, and closed-session finalization summary; `sessions` lists the bounded durable inventory (title/status/staleness/has_summary, optional status/task filter). `title` names a session (bounded, secret-redacted, idempotent). `close` finalizes a session with terminal status `completed`/`abandoned`, an optional end `reason`, and an optional bounded `summary` persisted as a schema-tagged snapshot for future recall — status and snapshot land atomically; repeated close returns the durable ending unchanged (never rewritten, no duplicate `session_ended` history); malformed/legacy snapshots degrade to "no summary", never an error; neither action creates context records or USER_CONFIRMED facts. Task history stays invisible without its task; provenance `historical-evidence`; history never enters `context` |
...
| `skill` | v1.3-dev (P4, `docs/evolution/P4_IMPLEMENTATION.md`; P10 selection/context/approval, `docs/evolution/P10_IMPLEMENTATION.md`) | `action` (discover \| propose \| inspect \| validate \| approve \| reject \| deprecate \| rollback \| health \| applicable \| skill_context \| request_approval \| respond \| detect_reuse \| detect_evolution \| validate_evolution \| compare_versions), `candidate_id?`, `skill_id?`, `name?`, `description?`/`purpose?`/`content?` (propose; description/purpose secret-redacted at write, content additionally secret-scanned at validation), `learning_candidate_id?` (propose from accepted P3 learning), `scope?` (project default \| task + `task_id?` \| global), `languages?`/`subsystems?` (applicability), `confidence?` (standalone only; capped below the approval floor), `status?`/`limit?` (discover), `reason?` (reject/deprecate; also bounded failure context for `health` recordings), `version?` (rollback), `success?` (health), `user_confirmed?` (approve requires true), `task?`/`keywords?` (applicable/skill_context: task relevance, never persisted), `request_id?`/`response?` (respond: approve \| reject \| modify \| defer), `modification?`/`modified_content?` (respond/modify: human instruction + optional replacement content), `question?` (request_approval), `from_version?`/`to_version?`/`weakness_kind?` (validate_evolution/compare_versions version comparison), `workspace_root?` | skill lifecycle with layered trust gates: proposals carry evidence citations (accepted learning only — weak/rejected learning refused at the store), `validate` runs the automated evaluation pass (candidate → evaluating → draft → validated), `approve` publishes only with `user_confirmed=true` AND store gates (validated status, confidence ≥ 0.60, workspace match, no name conflict, no stale version anchor) — the model can never self-approve; `applicable` ranks workspace-visible active skills deterministically (intent overlap + applicability metadata + scope/version/health/confidence, deprecated/out-of-scope excluded with audit reasons, bounded); `skill_context` builds the minimal first-class packet (required/optional/constraints/expected outputs, bounded excerpts — never a memory dump or full SKILL.md); `request_approval` emits `status: needs_input` + question + options for OpenCode's native UI (CodeBro owns request identity/validation/transitions/persistence/audit); `respond` consumes approve/reject/modify/defer single-use with replay + stale-content + scope + expiry guards (expired requests are never consumable — request again) — modify mints a revalidated successor lineage requiring fresh approval and supersedes the parent candidate, never silent publish; approval decisions persist as history evidence (`user_confirmed` for human answers, `ai_inferred` for model-initiated requests); publication is atomic (temp+fsync+rename), symlink-safe, read-before-write (external SKILL.md edits are conflicts, never overwritten), and DB rows commit in one transaction; published versions are immutable (unique `(skill_id, version_number)`); rollback publishes old content as a new version (no-op rollbacks refused); deprecation removes the artifact so OpenCode stops discovering it; `detect_reuse` (P11: mine repeated successful workflows into evidence-backed candidates) and `detect_evolution` (P13: mine recurring skill-linked failures into evidence-backed successor-version candidates) are explicitly invoked detectors — never scheduled, never publishing; `validate_evolution` (P15: compare two skill versions on attributed execution evidence with a conservative improvement verdict; alias `compare_versions`) is an explicitly invoked read-only comparison — never publishing, never rolling back; every action enforces workspace/scope visibility (task rows need `task_id`); mutating actions serialize on the workspace lock; CodeBro never executes skills — OpenCode discovers `~/.config/opencode/skills/<name>/SKILL.md` (or `$CODEBRO_SKILLS_DIR`) natively |
...
| `apply_change` | `path`, `old` (empty = create), `new` | single-file guarded mutation: boundary, traversal denial, symlink escape prevention, stale-content protection, ambiguity rejection; post-apply read-back (`edit_verification`) — a mismatch rolls back and errors; response `status=applied_unverified`, `verification_status=unverified` until a build/test passes, plus `recommended_tests` (edited-symbol linkage, cap 32) and invalidation advisory |
| `apply_changes` | `changes[]{path,old,new}` | multi-file transaction: validate all against current content → conflict pass re-checks staleness across the set → sequential apply with rollback on first failure; every file read back after apply, any mismatch rolls the whole set back and errors; all-or-nothing; same `applied_unverified` semantics |
| `sandbox_exec` | `command` (read-only build/test/lint policy), `working_directory?`, `timeout?`, `metadata?` | fail-closed execution; full evidence envelope below; raw execution — does NOT produce durable validation evidence |
| `sandbox_build` | same contract as sandbox_test (auto: cargo check/go build/npm-pnpm-yarn run build) | build/check verification; recorded durably per tree hash and returned as `execution_state` |
| `sandbox_test` | `command?`, `test_filter?` (targeted names; cargo/go/pytest runners), `expected_exit_code?`, `expected_success?`, `affected_fact_ids?` | auto-detected runner + pass/fail verification with violations, parsed `diagnostics`, coarse `classification`, `affected_modules`, `related_recent_changes` (in-session edit correlation), deterministic `root_cause` hypotheses on failures, and current-tree `execution_state` (`failed | verified | unverified | unknown`) |
...
```json
{
  ...
  "verification": {
    "verified", "summary", "violations", ["impacted_fact_ids"],
    ["diagnostics"], ["classification"], ["affected_modules"],
    ["prior_evidence"], ["related_recent_changes"], ["root_cause"],
    "execution_state": {
      "state": "failed | verified | unverified | unknown",
      "tree_hash?", ["unresolved_failures"], "unresolved_failures_total",
      ["inconclusive_failures"], "inconclusive_failures_total",
      "last_success?", "evidence_records", "note"
    }
  }
}
```
