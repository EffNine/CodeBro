...
| `skill` | v1.3-dev (P4, `docs/evolution/P4_IMPLEMENTATION.md`; P10 selection/context/approval, `docs/evolution/P10_IMPLEMENTATION.md`) | `action` (discover \| propose \| inspect \| validate \| approve \| reject \| deprecate \| rollback \| health \| applicable \| skill_context \| request_approval \| respond), `candidate_id?`, `skill_id?`, `name?`, `description?`/`purpose?`/`content?` (propose; description/purpose secret-redacted at write, content additionally secret-scanned at validation), `learning_candidate_id?` (propose from accepted P3 learning), `scope?` (project default \| task + `task_id?` \| global), `languages?`/`subsystems?` (applicability), `confidence?` (standalone only; capped below the approval floor), `status?`/`limit?` (discover), `reason?` (reject/deprecate), `version?` (rollback), `success?` (health), `user_confirmed?` (approve requires true), `task?`/`keywords?` (applicable/skill_context: task relevance, never persisted), `request_id?`/`response?` (respond: approve \| reject \| modify \| defer), `modification?`/`modified_content?` (respond/modify: human instruction + optional replacement content), `question?` (request_approval), `workspace_root?` | skill lifecycle with layered trust gates: proposals carry evidence citations (accepted learning only — weak/rejected learning refused at the store), `validate` runs the automated evaluation pass (candidate → evaluating → draft → validated), `approve` publishes only with `user_confirmed=true` AND store gates (validated status, confidence ≥ 0.60, workspace match, no name conflict, no stale version anchor) — the model can never self-approve; `applicable` ranks workspace-visible active skills deterministically (intent overlap + applicability metadata + scope/version/health/confidence, deprecated/out-of-scope excluded with audit reasons, bounded); `skill_context` builds the minimal first-class packet (required/optional/constraints/expected outputs, bounded excerpts — never a memory dump or full SKILL.md); `request_approval` emits `status: needs_input` + question + options for OpenCode's native UI (CodeBro owns request identity/validation/transitions/persistence/audit); `respond` consumes approve/reject/modify/defer single-use with replay + stale-content + scope guards — modify mints a revalidated successor lineage requiring fresh approval, never silent publish; approval decisions persist as history evidence (`user_confirmed` for human answers, `ai_inferred` for model-initiated requests); publication is atomic (temp+fsync+rename), symlink-safe, read-before-write (external SKILL.md edits are conflicts, never overwritten), and DB rows commit in one transaction; published versions are immutable (unique `(skill_id, version_number)`); rollback publishes old content as a new version (no-op rollbacks refused); deprecation removes the artifact so OpenCode stops discovering it; every action enforces workspace/scope visibility (task rows need `task_id`); mutating actions serialize on the workspace lock; CodeBro never executes skills — OpenCode discovers `~/.config/opencode/skills/<name>/SKILL.md` (or `$CODEBRO_SKILLS_DIR`) natively |
