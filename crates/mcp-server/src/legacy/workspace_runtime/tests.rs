...
use crate::legacy::workspace_runtime::{
...
fn environment_has_tool_lookup() {
    ...
    let profile = crate::legacy::workspace_runtime::EnvironmentProfile::default();
...
fn compute_diff_is_deterministic() {
    let a = crate::legacy::workspace_runtime::WorkspaceSnapshot {
        id: "a".into(),
        root: WorkspaceRoot::new(PathBuf::from("/")),
        captured_at_ms: 0,
        entries: vec![crate::legacy::workspace_runtime::SnapshotEntry {
    ...
    let b = a.clone();
    let d1 = crate::legacy::workspace_runtime::compute_diff(&a, &b);
    let d2 = crate::legacy::workspace_runtime::compute_diff(&a, &b);
    assert_eq!(d1, d2);
    assert!(d1.is_empty());
}
...
fn concurrent_diagnostics_recording() {
    use std::thread;
    let diag = Arc::new(crate::legacy::workspace_runtime::WorkspaceDiagnostics::new());
    let mut handles = Vec::new();
    for _ in 0..8 {
        let d = diag.clone();
        handles.push(thread::spawn(move || {
