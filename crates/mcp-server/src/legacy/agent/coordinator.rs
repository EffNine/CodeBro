...
use crate::legacy::agent::communication::MessageType;
use crate::legacy::agent::events::AgentEvent;
use crate::legacy::agent::grounding::{GroundedContext, GroundingAssembler};
use crate::legacy::agent::recovery::RecoveryEngine;
use crate::legacy::agent::router::TaskRouter;
use crate::legacy::agent::status::AgentStatus;
use crate::legacy::agent::subagent::{SubAgent, SubAgentContext, SubAgentResult};
...
pub struct AgentCoordinator {
    ...
    pub message_bus: crate::legacy::agent::communication::AgentMessageBus,
...
impl AgentCoordinator {
    pub fn new(max_agents: usize) -> Self {
        AgentCoordinator {
            ...
            message_bus: crate::legacy::agent::communication::AgentMessageBus::new(),
    ...
    pub async fn spawn_agent(
        &mut self,
        name: &str,
        _role: crate::legacy::agent::coordinator::AgentRole,
    ) -> bool {
    ...
    pub async fn send_message(
        ...
        content: &str,
        priority: crate::legacy::agent::communication::MessagePriority,
    ) -> String {
        self.message_bus
            .send(
                from,
                ...
                content,
                priority,
                crate::legacy::agent::communication::MessageChannel::Public,
                HashMap::new(),
            )
            .await
    }
            ...
            if agent != from {
                let _ = self
                    .send_message(
                        ...
                        msg_type.clone(),
                        content,
                        crate::legacy::agent::communication::MessagePriority::Normal,
                    )
                    .await;
            }
        }
    ...
    async fn run_task_grounded_with(
        ...
        for name in &agent_names {
            ...
            if let Some(agent) = agent_factory(name) {
                ...
                // `crate::legacy::tools::run_tool_pipeline`, which runs first and emits the
