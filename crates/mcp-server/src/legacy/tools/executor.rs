...
use crate::legacy::tools::{SmartToolRouter,ToolSelection};
...
pub fn run_tool_pipeline(task: &str, root: &Path) -> Result<PipelineResult> {
    ...
    let router = SmartToolRouter::new(crate::legacy::dispatcher::ToolDispatcher::new(
        crate::legacy::dispatcher::ToolRegistry::new(),
    ));
